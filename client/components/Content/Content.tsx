...
class Content extends React.Component<Props, State> {
    ...
    render = () => {
        ...
        // URL checking
        var urlParams: PostsAPI['url'] = fromUrlParams();
        if (postsOptions.pageNumber != urlParams.postsPageNumber
            || postsOptions.displayCount != urlParams.postsDisplayCount) {
            this.changePostsOptions({
                pageNumber: urlParams.postsPageNumber,
                displayCount: urlParams.postsDisplayCount,
            }, false);
            this.fetchPosts();
        }
        // Additional values
        ...
        return (
            <div id="content">
                            ...
                            { getRangeArray(pageButtonStart <= 1 + 1 ? 1 : pageButtonStart, pageButtonEnd >= pagesCount - 1 ? pagesCount : pageButtonEnd).map(value => (
                                <li key={value} className={classNames('page_number', value == postsOptions.pageNumber ? " selected" : " clickable")}>
                                    { value != postsOptions.pageNumber
                                        && (url = mergeUrlParams({ postsPageNumber: value, postsDisplayCount: postsOptions.displayCount })) != undefined
                                        && (
                                            <Link to={url} onClick={() => {
                                ...
                                return (
                                    <li key={i} className={`posts_display_count${elem == postsOptions.displayCount ? " selected" : " clickable"}`}>
                                        { elem != postsOptions.displayCount
                                            && (url = mergeUrlParams({ postsPageNumber: newPageNumber, postsDisplayCount: elem })) != undefined
                                            && (
                                                <Link to={url} onClick={() => {
                                                    this.changePostsOptions({
                ...
                { store.posts.map((elem, i) =>
                    <ContentPost key={store.posts.ids[i]} options={elem} />
                ) }
                { store.loggedIn && (
                    <ContentCreatePost />
                ) }
